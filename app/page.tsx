...
export default function Dashboard() {
    ...
    // Flag to avoid re-running data load after initial setup
    ...
    const refreshData = async (exerciseId?: number) => {
        ...
        setData(res);
        // Only auto-set targetExercise on initial load (targetExercise === 0)
        if (exerciseId === undefined && targetExercise === 0 && res.exercises.length > 0) {
            const defaultExId = await getDefaultExercise();
            if (defaultExId && res.exercises.some(e => e.id === defaultExId)) {
                setTargetExercise(defaultExId);
            } else {
                setTargetExercise(res.exercises[0].id);
            }
        }
        if (!logExerciseId && res.exercises.length > 0) setLogExerciseId(res.exercises[0].id.toString());
    };

        // Load profile, goal, and set target exercise based on default from DB (only after data is loaded and user known)
        // Use a ref to prevent multiple simultaneous executions
        const isLoadingUserData = React.useRef(false);

        // Load profile, weight goal, and default exercise when user changes
        useEffect(() => {
            const loadUserData = async () => {
                // Only run if we have a user name and the data is loaded
                ...
                try {
                    // Load profile and goal in parallel
                    ...
                    // Set default exercise only if it's different from current targetExercise
                    let newExerciseId = null;
                    if (defaultExId && data.exercises.some(e => e.id === defaultExId)) {
                        newExerciseId = defaultExId;
                    } else if (data.exercises.length > 0) {
                        newExerciseId = data.exercises[0].id;
                    }

                    if (newExerciseId !== null && targetExercise !== newExerciseId) {
                        setTargetExercise(newExerciseId);
                        // Refresh data with the new exercise ID
                        const freshData = await getDashboardData(newExerciseId);
                        setData(freshData);
                        if (!logExerciseId && freshData.exercises.length > 0) {
            ...
            loadUserData();
        }, [currentUserName, data]); // data is needed, but we prevent re-runs with the ref

        // When targetExercise changes from user dropdown, refresh data with new exercise
        useEffect(() => {
            if (isInitialLoadDone && data && targetExercise !== data.activeExerciseId) {
                refreshData(targetExercise);
            }
        }, [targetExercise]);
